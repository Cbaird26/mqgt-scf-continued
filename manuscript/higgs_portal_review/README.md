...
## Subsequent development (Notes 15–22; not silently backported into dated papers)
...
`safe_omnes_manifest.py` is an original offline size/Git blob-digest verifier for four pinned upstream Omnès files; its separate raw SHA-256 receipt and escaped byte sample require actual local files. `test_research_note_21.py` has 15 passing local tests using **synthetic file fixtures and algebra only**. The original numerical Omnès files remain unavailable/unparsed; no hadronic central width or tighter lifetime has been computed. Once all four files have been independently obtained, run `python3 safe_omnes_manifest.py /absolute/path/to/hipsofcobra/input`. All prior physics, review, licensing and publication gates remain open.

**Note 22** (`RESEARCH_NOTE_22_COMPRESSED_SPECTRAL_WINDOW_BENCHMARK.md`) defines a *different, illustrative* companion benchmark with `m1=10 GeV` and `m2=11.5 GeV`, forcing `sqrt(s)<=1.5 GeV` in `s2->s1+h*->s1+X`. This removes the *unmodeled 2–15 GeV tail* for the pion/kaon exclusive channels **if** the independently verified shipped data cover the full 1.5-GeV domain; it neither retrieves the four files nor completes the decay inventory. The new point's leptonic-only `c*tau2 <= ~645 km` is a very weak **upper bound, not a lifetime prediction**, and MUST NOT be confused with the previous, different 15-GeV-gap point's `c*tau2 <= ~17.06 cm`. `near_threshold_benchmark.py` and `test_research_note_22.py` have 13 passing local algebra/toy-integration tests, with a separate direct-variable SciPy spot-check. The research papers remain dated drafts; no collider exclusion, hadronic width, total lifetime or physical field detection is established.
